...
```{r}
scatter.raw$cluster <- fct_collapse(scatter.raw$cluster, doublet = setdiff(levels(scatter.raw$cluster), 1:3))
scatter.raw$cluster <- fct_recode(scatter.raw$cluster, HCEC = "1", RPE1 = "2", HPNE = "3")
...
```{r}
exp4.wc.gmm.heatmap <- 
    assayHeatmap(exp4, alt.exp = "smoothedCopyNumberByChr", assay = "gmmCopyNumber", split.col.by = "feature.id", split.row.by = "cluster", column_title_rot = 0, color.custom = gmm.anno.scale, heatmap_legend_param = list(title_position = "lefttop-rot", border = "black"), name = "Copy Number")
exp4.wc.gmm.heatmap
```
...
pdf(file = here("figures", "figure-i", "exp04.wc.gmm.heatmap.pdf"), 
    width = 7.90, height = 3.75)
draw(exp4.wc.gmm.heatmap, use_raster = T, raster_quality = 7)
dev.off()
```
...
exp4.arm.gmm.heatmap <- 
    assayHeatmap(exp4, alt.exp = "smoothedCopyNumberByArm", assay = "gmmCopyNumber", split.col.by = "feature.id", split.row.by = "cluster", column_title_rot = 90, color.custom = gmm.anno.scale, heatmap_legend_param = list(title_position = "lefttop-rot", border = "black"), name = "Copy Number")
exp4.arm.gmm.heatmap
```
...
ggplot(combined.arm.results, aes(x = feature.id, y = delta.acc)) +
    ...
    scale_y_continuous(limits = c(-15, 30), expand = c(0,0,0,0)) +
