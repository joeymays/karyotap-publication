...
ggplot(combined.results, aes(x = feature.id, y = delta.acc)) +
    ...
    labs(x = "Chromosome", y = "\u0394 Sensitivity (pp)") +
...
ggplot(combined.results, aes(x = delta.probes, y = delta.acc)) +
    ...
    theme_bw_custom + 
    theme(panel.border = element_blank(), axis.line = element_line(), panel.grid.minor.y = element_blank()) +
    labs(x = "\u0394 Probe Number", y = "\u0394 Sensitivity (pp)") +
...
ggplot(sensitivity.compare, aes(x=feature.id, y=delta.tpr, color = cn.sim.class)) +
    ...
    labs(y = "\u0394 Theoretical Sensitivity (pp)", x = "Chromosome", color = "Copy Number")
tpr.wc.dots.compare
```
...
combined.arm.results <- combined.arm.results %>% mutate(delta.acc = pct.concordance.v3 - pct.concordance.v1, 
                            delta.probes = n.probes.v3 - n.probes.v1)
```
...
ggplot(combined.arm.results, aes(x = feature.id, y = delta.acc)) +
    ...
    labs(x = "Chromosome Arm", y = "\u0394 Sensitivity (pp)") +
    scale_y_continuous(limits = c(-15, 30), expand = c(0,0,0,0)) +
    geom_text(aes(y = 28, label = sprintf("%+2d", combined.arm.results$delta.probes)), size = 6/.pt) +
    geom_hline(yintercept = 0) +
