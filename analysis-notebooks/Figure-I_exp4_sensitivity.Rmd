---
title: "Figure I - Exp4 Sensitivity Metrics"
author: "Joey Mays"
date: '2023-04-18'
output:
  pdf_document:
...
```{r}
here::i_am("analysis-notebooks/Figure-I_exp4_sensitivity.Rmd")
```
...
```{r}
dir.create(here("figures", "figure-i"), showWarnings = F)
fig.scalar = 1.375
...
```{r}
scatter.raw <- reducedDim(altExp(exp4.full, "alleleFrequency"), "UMAP")
scatter.raw$cluster <- colData(exp4.full)$cluster
```
...
```{r}
saveRDS(exp4.umap, here("figures", "figure-i", "exp04.af.umap.RDS"))
```

```{r}
#Rasterize points so Illustrator doesn't complain
rasterize(exp4.umap, layers = "point", dpi = 300)
```

```{r}
pdf(file = here("figures", "figure-i", "exp04.af.umap.pdf"), 
    width = 3.2, height = 2.0)
#Rasterize points so Illustrator doesn't complain
#rasterize(exp4.umap, layers = "Point", dpi = 1200)
exp4.umap
dev.off()
```
...
```{r}
y.data <- getTidyData(exp4, alt.exp = "chrYCounts")
y.data$feature.id <- str_split_fixed(y.data$feature.id, pattern = "_", n = 2)[,2]
...
```{r}
saveRDS(ychr.plot, here("figures", "figure-i", "chrY.probes.RDS"))
```
...
```{r}
row.annotation.data <- getTidyData(exp4, assay = "copyNumber") %>%
...
names(color.vector) <- c("RPE1", "HCEC", "HPNE")
...
```{r}
exp4.wc.gmm.heatmap <- 
    assayHeatmap(exp4, alt.exp = "smoothedCopyNumberByChr", assay = "gmmCopyNumber", split.col.by = "feature.id", split.row.by = "cluster", column_title_rot = 0, color.custom = gmm.anno.scale, heatmap_legend_param = list(title_position = "topcenter", border = "black", labels = c("4+", "3", "2", "1"), at = 4:1), name = "Single Cell\nCopy Number\nCall")
exp4.wc.gmm.heatmap
```

```{r}
saveRDS(exp4.wc.gmm.heatmap, file = here("figures", "figure-i", "exp04.wc.gmm.heatmap.RDS"))
```

```{r}
pdf(file = here("figures", "figure-i", "exp04.wc.gmm.heatmap.pdf"), 
    width = 7.90, height = 3.75)
ht_opt("HEATMAP_LEGEND_PADDING" = unit(10, "mm"))
draw(exp4.wc.gmm.heatmap, use_raster = T, raster_quality = 7, padding = unit(c(2,2,2,8), "mm"))
ht_opt("HEATMAP_LEGEND_PADDING" = unit(10, "mm"))
dev.off()
```
...
```{r}
raw.ploidy.wc <- round(t(assay(altExp(exp4, "smoothedCopyNumberByChr"), "smoothedCopyNumber")), 3)
colnames(raw.ploidy.wc) <- paste0("chr", colnames(raw.ploidy.wc))
raw.ploidy.wc <- cbind(colData(exp4)[,"cluster", drop = F], raw.ploidy.wc) 
raw.ploidy.wc <- raw.ploidy.wc %>% as.data.frame() %>% rownames_to_column("cell.barcode")
```

```{r, eval = F}
write.csv(x = raw.ploidy.wc, file = here("figures", "figure-i", "exp04.wc.smooth.heatmap.raw.csv"), 
          quote = F, row.names = F)
```
...
```{r}
exp4.arm.smoothed.heatmap <- assayHeatmap(exp4, alt.exp = "smoothedCopyNumberByArm", assay = "smoothedCopyNumber", split.col.by = "feature.id", split.row.by = "cluster", annotate.row.by = "cluster", column_title_rot = 90, color.custom = circlize::colorRamp2(c(1, 1.5, 2, 2.5, 3, 4), c("#2c7bb6", "#ffffff", "#ffffff", "#ffffff", "#fdae61", "#d7191c")), left_annotation = row.annotation, heatmap_legend_param = list(title_position = "lefttop-rot", border = "black"), name = "Copy Number")
exp4.arm.smoothed.heatmap
```

```{r}
pdf(file = here("figures", "figure-i", "exp04.arm.smoothed.heatmap.pdf"), 
    width = 7.90, height = 3.75)
draw(exp4.arm.smoothed.heatmap, use_raster = T, raster_quality = 7)
dev.off()
```
...
rownames(altExp(exp4.hm, "smoothedCopyNumberByArm")) <- str_split_i(rownames(altExp(exp4.hm, "smoothedCopyNumberByArm")), "chr", 2)
```
...
```{r}
exp4.arm.gmm.heatmap <- 
    assayHeatmap(exp4.hm, alt.exp = "smoothedCopyNumberByArm", assay = "gmmCopyNumber", split.col.by = "feature.id", split.row.by = "cluster", column_title_rot = 45, color.custom = gmm.anno.scale, heatmap_legend_param = list(title_position = "topcenter", border = "black", labels = c("4+", "3", "2", "1"), at = 4:1), name = "Single Cell\nCopy Number\nCall")
exp4.arm.gmm.heatmap
```

```{r}
saveRDS(exp4.arm.gmm.heatmap, file = here("figures", "figure-i", "exp04.arm.gmm.heatmap.RDS"))
```


```{r}
pdf(file = here("figures", "figure-i", "exp04.arm.gmm.heatmap.pdf"), 
    width = 7.90, height = 3.75)
draw(exp4.arm.gmm.heatmap, use_raster = T, raster_quality = 7)
dev.off()
```
...
```{r}
raw.ploidy.arm <- round(t(assay(altExp(exp4, "smoothedCopyNumberByArm"), "smoothedCopyNumber")), 3)
raw.ploidy.arm <- cbind(colData(exp4)[,"cluster", drop = F], raw.ploidy.arm) 
raw.ploidy.arm <- raw.ploidy.arm %>% as.data.frame() %>% rownames_to_column("cell.barcode")
```

```{r, eval = F}
write.csv(x = raw.ploidy.arm, file = here("figures", "figure-i", "exp04.arm.smooth.heatmap.raw.csv"), 
          quote = F, row.names = F)
```
...
```{r}
rpe1.data <- getTidyData(exp4, "smoothedCopyNumberByChr", "gmmCopyNumber") %>% filter(cluster == "RPE1", feature.id != 10) %>% select(!c("total.reads", "cluster"))
...
```{r}
n.probes.data <- fct_count(rowData(exp4)$chr)
colnames(n.probes.data) <- c("feature.id", "n.probes")
```
...
```{r}
saveRDS(rpe1.wc.sensitivity, here("figures", "figure-i", "exp04.rpe1.sensitivity.wc.RDS"))
```

```{r}
saveRDS(rpe1.data.results %>% left_join(n.probes.data, by = "feature.id"), file = here("datasets", "exp04.rpe1.accuracy.rates.RDS"))
```

### Probe Efficiency

Probes for Chr22 have larger residuals on average than other chromosomes with similar coverage. 

```{r}
getTidyData(exp4, assay = "copyNumber", feature.id.as.factor = F)  %>% filter(cluster == "RPE1", chr != "10") %>% mutate(residual = (2 - copyNumber)) %>% select(chr, feature.id, residual) %>% filter(chr %in% c(16, 17, 20, 21, 22, "X")) %>% group_by(chr, feature.id) %>% summarize(mr = mean(residual)) %>% summarise(mean(mr))
```

### Panel v1 Comparison

```{r}
n.probes <- fct_count(rowData(exp4)$chr)
colnames(n.probes) <- c("feature.id", "n.probes")
rpe1.data.results <- rpe1.data.results %>% left_join(n.probes, by = "feature.id")
```

```{r}
exp1.wc.results <- readRDS(here("datasets", "rpe1.accuracy.rates.RDS"))
```

```{r}
combined.results <- rpe1.data.results %>% inner_join(exp1.wc.results %>% select(feature.id, pct.concordance, n.probes), by = "feature.id", suffix = c(".v3", ".v1")) %>% select(!c("ploidy.concordance", "n"))
```

```{r}
combined.results <- combined.results %>% mutate(delta.acc = pct.concordance.v3 - pct.concordance.v1, delta.probes = n.probes.v3 - n.probes.v1)
```
...
ggplot(combined.results, aes(x = feature.id, y = delta.acc)) +
    ...
    geom_text(aes(y = 28, label = sprintf("%+2d", delta.probes)), size = 7/.pt) +
    geom_hline(yintercept = 0)
rpe1.wc.sensitivity.change
```

```{r}
saveRDS(rpe1.wc.sensitivity.change, here("figures", "figure-i", "exp04.rpe1.sensitivity.wc.change.RDS"))
```
...
```{r}
saveRDS(rpe1.wc.sensitivity.change.lm, here("figures", "figure-i", "exp04.rpe1.sensitivity.wc.change.regression.RDS"))
```

Chromosomes with fewer probes in v1 generally had larger accuracy increase. 

```{r}
ggplot(combined.results, aes(x = n.probes.v1, y = delta.acc)) +
    geom_point() + 
...
```{r}
exp1.gmm.metrics <- readRDS(here("datasets", "exp1.gmm.metrics.RDS"))
```

```{r}
exp4.gmm.metrics <- calcTruePosRate(exp4, "chr") %>% filter(cn.sim.class != "sim_cn6")
exp4.gmm.metrics <- exp4.gmm.metrics %>% left_join(n.probes, by = "feature.id")
exp4.gmm.metrics <- exp4.gmm.metrics %>% mutate(false.neg.p = false.neg.p * 100, true.pos.p = true.pos.p * 100)
```

```{r}
 saveRDS(exp4.gmm.metrics, here("datasets", "exp4.gmm.metrics.RDS"))
```

```{r}
tpr.wc.bars <- 
ggplot(exp4.gmm.metrics, aes(x = feature.id, y = true.pos.p)) +
...
```{r}
saveRDS(tpr.wc.bars, here("figures", "figure-i", "exp04.rpe1.tpr.wc.RDS"))
```

```{r}
tpr.wc.nprobes <- 
ggplot(exp4.gmm.metrics, aes(x=n.probes, y=true.pos.p, color = cn.sim.class)) +
    ...
    scale_x_continuous(limits = c(11,22), breaks = seq(12,22,2))
tpr.wc.nprobes
```
...
```{r}
sensitivity.compare <- inner_join(exp1.gmm.metrics %>% select(feature.id, cn.sim.class, true.pos.p), 
           exp4.gmm.metrics %>% select(feature.id, cn.sim.class, true.pos.p),
           by = c("feature.id", "cn.sim.class"), suffix = c(".exp1", ".exp4")) %>% 
    mutate(delta.tpr = true.pos.p.exp4 - true.pos.p.exp1) %>% 
...
```{r}
tpr.wc.dots.compare <- 
ggplot(sensitivity.compare, aes(x=feature.id, y=delta.tpr, color = cn.sim.class)) +
    geom_point(alpha = 0.8) +
    theme_bw_custom +
    theme(legend.position = "top") +
    scale_color_viridis_d(labels = 1:5) +
    labs(y = "\u0394 Simualted Sensitivity (pp)", x = "Chromosome", color = "Copy Number")
tpr.wc.dots.compare
```

```{r}
tpr.wc.nprobes.compare <- 
ggplot(sensitivity.compare, aes(x=delta.probes, y=delta.tpr, color = cn.sim.class)) +
    geom_point(alpha = 0.5) +
    geom_smooth(se = F, span = 1, method = "lm") + 
    theme_bw() +
    theme(legend.position = "top") +
    scale_color_viridis_d(labels = 1:5) +
    labs(y = "\u0394 Simualted Sensitivity (pp)", x = "\u0394 Number of Probes", color = "Copy Number") +
    scale_x_continuous(limits = c(-6, 8), breaks = seq(-6, 8, 2))
tpr.wc.nprobes.compare
```

```{r}
sensitivity.compare.avg <- sensitivity.compare %>% group_by(cn.sim.class) %>% summarize(mean.tpr1 = mean(true.pos.p.exp1), mean.tpr4 = mean(true.pos.p.exp4))
sensitivity.compare.avg$cn.sim.class.label <- paste0("Copy Number = ", 1:5)
```

```{r}
sensitivity.compare.plot <- 
ggplot(sensitivity.compare.avg, aes(x = mean.tpr1, y = mean.tpr4, color = cn.sim.class, label = cn.sim.class.label)) +
    geom_point() +
    ...
    labs(x = "Panel Version 1\nMean Theoretical Sensitivity (%)", y = "Panel Version 2\nMean Theoretical Sensitivity (%)")
sensitivity.compare.plot
``` 

```{r}
saveRDS(sensitivity.compare.plot, here("figures", "figure-i", "exp04.rpe1.sensitivity.compare.RDS"))
```
...
```{r}
exp4.gmm.specificity <- calcTrueNegRate(exp4) %>% mutate(tnr = tnr * 100) %>% filter(cn.sim.class != "sim_cn6")
exp4.gmm.specificity <- exp4.gmm.specificity %>% left_join(n.probes, by = "feature.id")

ggplot(exp4.gmm.specificity, aes(x = feature.id, y = tnr)) +
    geom_col(aes(fill = cn.sim.class), color = "black", width = 0.85) +
    facet_wrap(cn.sim.class ~., ncol = 1) +
    theme_classic_custom +
...
```{r}
ggplot(exp4.gmm.specificity, aes(x=n.probes, y=tnr, color = cn.sim.class)) +
...
```{r}
rpe1.bcs <- colData(exp4) %>% as_tibble %>% filter(cluster == "RPE1") %>% pull(cell.barcode)
rpe1.bcs.scrambled <- sample(rpe1.bcs, size = length(rpe1.bcs), replace = F)
rpe1.index <- cut_number(seq_along(rpe1.bcs.scrambled), n = 5)
rpe1.index <- rpe1.index %>% fct_recode("fold1" = "[1,127]", "fold2" = "(127,253]", "fold3" = "(253,379]", "fold4" = "(379,505]", "fold5" = "(505,631]")
cv.folds <- data.frame(cell.barcode = rpe1.bcs.scrambled, cv.fold = rpe1.index)
```

```{r}
exp4.cv <- exp4
control.copy.number <- generateControlCopyNumberTemplate(exp4, copy.number = 2, "RPE1")
...
cv.results <- map(levels(cv.folds$cv.fold), function(x){
    ...
    exp4.cv <- calcGMMCopyNumber(exp4.cv, cell.barcodes = training.bcs, control.copy.number = control.copy.number)
    
    rpe1.cv.data <- getTidyData(exp4.cv, "smoothedCopyNumberByChr", "gmmCopyNumber") %>% filter(cluster == "RPE1", feature.id != 10, !cell.barcode %in% training.bcs) %>% select(!c("total.reads", "cluster"))
...
cv.results %>% group_by(feature.id) %>% summarize(mean.pct = mean(pct.concordance), mad = mad(pct.concordance), mean.ad = DescTools::MeanAD(pct.concordance)) %>% print() %>% 
    ...
    scale_y_continuous(limits = c(50, 100), expand = c(0,0,0,0)) +
    labs(y = "Accuracy (%)", x = "Chromosome") + 
...
```
```{r}
saveRDS(wc.sensitivity.mad, here("figures", "figure-i", "exp04.rpe1.sensitivity.wc.mad.RDS"))
```
...
control.copy.number <- generateControlCopyNumberTemplate(exp4, copy.number = 2, "RPE1")
control.copy.number["chr10q", "copy.number"] <- 3
exp4.3comp <- calcGMMCopyNumber(exp4, cell.barcodes = rpe1.bcs, control.copy.number = control.copy.number, model.components = 1:3)
exp4.gmm.metrics.3 <- calcTruePosRate(exp4.3comp, "chr") %>% filter(cn.sim.class != "sim_cn6")
exp4.gmm.metrics.3 <- exp4.gmm.metrics.3 %>% left_join(n.probes, by = "feature.id")
exp4.gmm.metrics.3 <- exp4.gmm.metrics.3 %>% mutate(false.neg.p = false.neg.p * 100, true.pos.p = true.pos.p * 100)
```

```{r}
exp4.gmm.metrics.3.avg <- exp4.gmm.metrics.3 %>% group_by(cn.sim.class) %>% summarize(mean.tpr = mean(true.pos.p), .groups = "drop") %>% print()
```

```{r}
exp4.3comp.wc.sens <- 
ggplot(exp4.gmm.metrics.3, aes(x = feature.id, y = true.pos.p)) +
    geom_col(aes(fill = cn.sim.class), color = "black", width = 0.85) +
    facet_wrap(cn.sim.class ~., ncol = 1) +
    theme_classic_custom +
    theme(legend.position = "none", panel.grid.major.y = element_line(), 
    ...
    geom_text(data = exp4.gmm.metrics.3.avg, aes(label = glue::glue("avg: {round(mean.tpr, 0)}%"), x = 22, y = 140), size = 6/.pt, inherit.aes = F)
exp4.3comp.wc.sens
```

```{r}
saveRDS(exp4.3comp.wc.sens, here("figures", "figure-i", "exp04.rpe1.tpr.wc.3comp.RDS"))
```

```{r}
ggplot(exp4.gmm.metrics.3, aes(x=n.probes, y=true.pos.p, color = cn.sim.class)) +
    geom_point(alpha = 0.5, position = position_jitter(width = 0.25)) + 
    theme_classic_custom +
    theme(legend.position = "top", panel.grid.major = element_line()) +
    theme(legend.box.margin = margin(0, 10, 0, 10), legend.margin = margin(0), plot.margin = unit(c(0, 5.5, 5.5, 5.5), "pt")) +
    ...
    labs(x = "Number of Probes", y = "Theoretical Sensitivity (%)", color = "Copy Number") +
    scale_x_continuous(limits = c(10,24), breaks = seq(4,24,2)) +
    scale_y_continuous(limits = c(70, 101), expand = c(0,0,0,2))
```

```{r}
exp4.gmm.metrics.3.spec <- calcTrueNegRate(exp4.3comp) %>% mutate(tnr = tnr * 100)
exp4.gmm.metrics.3.spec <- exp4.gmm.metrics.3.spec %>% left_join(n.probes, by = "feature.id")

ggplot(exp4.gmm.metrics.3.spec, aes(x = feature.id, y = tnr)) +
    geom_col(aes(fill = cn.sim.class), color = "black", width = 0.85) +
    facet_wrap(cn.sim.class ~., ncol = 1) +
    theme_classic_custom +
    theme(legend.position = "none", panel.grid.major.y = element_line(), 
...
```{r}
ggplot(exp4.gmm.metrics.3.spec, aes(x=n.probes, y=tnr, color = cn.sim.class)) +
    geom_point(alpha = 0.5, position = position_jitter(width = 0.25)) + 
    theme_classic_custom +
    theme(legend.position = "top", panel.grid.major = element_line()) +
    theme(legend.box.margin = margin(0, 10, 0, 10), legend.margin = margin(0), plot.margin = unit(c(0, 5.5, 5.5, 5.5), "pt")) +
    ...
    labs(x = "Number of Probes", y = "Theoretical Specificity (%)", color = "Copy Number") +
    scale_x_continuous(limits = c(10,23), breaks = seq(4,24,2)) +
    scale_y_continuous(limits = c(80, 100), expand = c(0,0,0,1))
```
...
```{r}
rpe1.data.arm <- getTidyData(exp4, "smoothedCopyNumberByArm", "gmmCopyNumber") %>% filter(cluster == "RPE1") %>% select(!c("total.reads", "cluster"))
...
```{r}
n.probes.arm.data <- fct_count(rowData(exp4)$arm)
colnames(n.probes.arm.data) <- c("feature.id", "n.probes")
```
...
```{r}
saveRDS(rpe1.arm.sensitivity, here("figures", "figure-i", "exp04.rpe1.accuracy.arm.RDS"))
```

```{r}
saveRDS(rpe1.data.arm.results %>% left_join(n.probes.arm.data, by = "feature.id"), file = here("datasets", "exp04.rpe1.arm.accuracy.rates.RDS"))
```

### Panel v1 Comparison

```{r}
n.probes.arm <- fct_count(rowData(exp4)$arm)
...
```{r}
exp1.arm.results <- readRDS(here("datasets", "rpe1.arm.accuracy.rates.RDS"))
```

```{r}
combined.arm.results <- rpe1.data.arm.results %>% inner_join(exp1.arm.results %>% select(feature.id, pct.concordance, n.probes), by = "feature.id", suffix = c(".v3", ".v1")) %>% select(!c("ploidy.concordance", "n"))
```

```{r}
combined.arm.results <- combined.arm.results %>% mutate(delta.acc = pct.concordance.v3 - pct.concordance.v1, 
                            delta.probes = n.probes.v3 - n.probes.v1) %>% mutate(feature.label = str_sub(feature.id, 4,-1))
```
...
ggplot(combined.arm.results, aes(x = feature.id, y = delta.acc)) +
    ...
    scale_y_continuous(limits = c(-20, 30), expand = c(0,0,0,0)) +
...
```{r}
saveRDS(rpe1.arm.sensitivity.change, here("figures", "figure-i", "exp04.rpe1.sensitivity.arm.change.RDS"))
```
...
```{r}
ggplot(combined.arm.results, aes(x = n.probes.v1, y = delta.acc)) +
    geom_point() + 
    geom_smooth(method = "lm")
```
...
```{r}
exp1.gmm.metrics.arms <- readRDS(here("datasets", "exp1.gmm.metrics.arms.RDS"))
```

```{r}
exp4.gmm.metrics.arms <- calcTruePosRate(exp4, "arm") %>% filter(cn.sim.class != "sim_cn6")
exp4.gmm.metrics.arms <- exp4.gmm.metrics.arms %>% left_join(n.probes.arm, by = "feature.id")
exp4.gmm.metrics.arms <- exp4.gmm.metrics.arms %>% mutate(false.neg.p = false.neg.p * 100, true.pos.p = true.pos.p * 100)
```

```{r}
 saveRDS(exp4.gmm.metrics.arms, here("datasets", "exp4.gmm.metrics.arms.RDS"))
```

```{r}
tpr.arm.bars <- 
ggplot(exp4.gmm.metrics.arms, aes(x = feature.id, y = true.pos.p)) +
...
```{r}
saveRDS(tpr.arm.bars, here("figures", "figure-i", "exp04.rpe1.tpr.arm.RDS"))
```

```{r}
tpr.arm.nprobes <- 
    ggplot(exp4.gmm.metrics.arms, aes(x=n.probes, y=true.pos.p, color = cn.sim.class)) +
    geom_point(alpha = 0.5) + 
...
```{r}
sensitivity.compare.arms <- inner_join(exp1.gmm.metrics.arms %>% select(feature.id, cn.sim.class, true.pos.p), 
           exp4.gmm.metrics.arms %>% select(feature.id, cn.sim.class, true.pos.p),
           by = c("feature.id", "cn.sim.class"), suffix = c(".exp1", ".exp4")) %>% 
    mutate(delta.tpr = true.pos.p.exp4 - true.pos.p.exp1) %>% 
...
```{r}
tpr.arms.dots.compare <- 
ggplot(sensitivity.compare.arms, aes(x=feature.id, y=delta.tpr, color = cn.sim.class)) +
    geom_point(alpha = 0.8) +
    theme_bw() +
    theme(legend.position = "top", axis.text.x = element_text(angle = 45, hjust = 1)) +
    scale_color_viridis_d(labels = 1:5) +
    labs(y = "\u0394 Theoretical Sensitivity (pp)", x = "Chromosome Arm", color = "Copy Number")
tpr.arms.dots.compare
```

```{r}
tpr.arms.nprobes.compare <- 
ggplot(sensitivity.compare.arms, aes(x=delta.probes, y=delta.tpr, color = cn.sim.class)) +
    geom_point(alpha = 0.5) +
    geom_smooth(se = F, span = 1, method = "lm") + 
    theme_bw() +
    theme(legend.position = "top") +
    scale_color_viridis_d(labels = 1:5) +
    labs(y = "\u0394 Theoretical Sensitivity (pp)", x = "\u0394 Number of Probes", color = "Copy Number") +
    scale_x_continuous(limits = c(-3, 7), breaks = c(-3, 0, 3, 6))
tpr.arms.nprobes.compare
```

```{r}
sensitivity.compare.avg.arms <- sensitivity.compare.arms %>% group_by(cn.sim.class) %>% summarize(mean.tpr1 = mean(true.pos.p.exp1), mean.tpr4 = mean(true.pos.p.exp4))
sensitivity.compare.avg.arms$cn.sim.class.label <- paste0("Copy Number = ", 1:5)
```

```{r}
sensitivity.compare.plot.arms <- 
ggplot(sensitivity.compare.avg.arms, aes(x = mean.tpr1, y = mean.tpr4, color = cn.sim.class, label = cn.sim.class.label)) +
    geom_point() +
    geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red") + 
    theme_classic_custom +
    theme(panel.grid.major = element_line()) + 
...
```{r}
saveRDS(sensitivity.compare.plot.arms, here("figures", "figure-i", "exp04.rpe1.sensitivity.compare.arms.RDS"))
```
...
cv.results.arms <- map(levels(cv.folds$cv.fold), function(x){
    ...
    training.bcs <- cv.folds %>% filter(cv.fold != x) %>% pull(cell.barcode)
    exp4.cv <- suppressMessages(calcGMMCopyNumber(exp4.cv, cell.barcodes = training.bcs, control.copy.number = control.copy.number))
    
    rpe1.cv.data <- getTidyData(exp4.cv, "smoothedCopyNumberByArm", "gmmCopyNumber") %>% filter(cluster == "RPE1", !cell.barcode %in% training.bcs) %>% select(!c("total.reads", "cluster"))
    rpe1.cv.data$known.ploidy <- 2
...
cv.results.arms %>% group_by(feature.id, feature.label) %>% summarize(mean.pct = mean(pct.concordance), mad = mad(pct.concordance), mean.ad = DescTools::MeanAD(pct.concordance), .groups = "drop") %>% print() %>% 
    ...
    scale_y_continuous(limits = c(50, 100), expand = c(0,0,0,0), breaks = seq(50, 100, 10)) +
    scale_x_discrete(labels = cv.results.arms$feature.label) +
    labs(y = "Accuracy (%)", x = "Chromosome Arm") + 
...
```{r}
saveRDS(arm.sensitivity.mad, here("figures", "figure-i", "exp04.rpe1.sensitivity.arm.mad.RDS"))
```

### 3 Component

```{r}
exp4.gmm.metrics.arm.3comp <- calcTruePosRate(exp4.3comp, "arm")
exp4.gmm.metrics.arm.3comp <- exp4.gmm.metrics.arm.3comp %>% left_join(n.probes.arm, by = "feature.id")
exp4.gmm.metrics.arm.3comp <- exp4.gmm.metrics.arm.3comp %>% mutate(false.neg.p = false.neg.p * 100, true.pos.p = true.pos.p * 100)
exp4.gmm.metrics.arm.3comp$feature.label <- exp4.gmm.metrics.arm.3comp$feature.id %>% str_sub(4,-1)
```

```{r}
exp4.gmm.metrics.arm.avg.3comp <- exp4.gmm.metrics.arm.3comp %>% group_by(cn.sim.class) %>% summarize(mean.tpr = mean(true.pos.p), .groups = "drop") %>% print()
```

```{r}
exp4.3comp.arm.sens <- 
ggplot(exp4.gmm.metrics.arm.3comp, aes(x = feature.id, y = true.pos.p)) +
    geom_col(aes(fill = cn.sim.class), color = "black", width = 0.75) +
    facet_wrap(cn.sim.class ~., ncol = 1) +
    theme_classic_custom +
    theme(legend.position = "none", panel.grid.major.y = element_line(),
    ...
    labs(x = "Chromosome Arm", y = "Theoretical Sensitivity (%)") +
    scale_y_continuous(breaks = c(0, 50, 100), expand = c(0,0,0,0)) +
    scale_x_discrete(labels = exp4.gmm.metrics.arm.3comp$feature.label) +
    geom_text(aes(label=round(true.pos.p,0)), vjust = -1, size = 6/.pt) +
    geom_text(data = exp4.gmm.metrics.arm.avg.3comp, aes(label = glue::glue("avg: {round(mean.tpr, 0)}%"), x = 38, y = 120), size = 6/.pt, inherit.aes = F)
exp4.3comp.arm.sens
```

```{r}
saveRDS(exp4.3comp.arm.sens, here("figures", "figure-i", "exp04.rpe1.tpr.arm.3comp.RDS"))
```

```{r}
ggplot(exp4.gmm.metrics.arm.3comp, aes(x=n.probes, y=true.pos.p, color = cn.sim.class)) +
    geom_point(alpha = 0.5, position = position_jitter(width = 0.25)) + 
    theme_classic_custom +
    theme(legend.position = "top", panel.grid.major = element_line()) +
    theme(legend.box.margin = margin(0, 10, 0, 10), legend.margin = margin(0), plot.margin = unit(c(0, 5.5, 5.5, 5.5), "pt")) +
...
```{r}
exp4.gmm.specificity.arm.3comp <- calcTrueNegRate(exp4.3comp, chromosome.scope = "arm") %>% mutate(tnr = tnr * 100)
exp4.gmm.specificity.arm.3comp <- exp4.gmm.specificity.arm.3comp %>% left_join(n.probes.arm, by = "feature.id")

ggplot(exp4.gmm.specificity.arm.3comp, aes(x = feature.id, y = tnr)) +
    geom_col(aes(fill = cn.sim.class), color = "black", width = 0.85) +
    facet_wrap(cn.sim.class ~., ncol = 1) +
    theme_classic_custom +
    theme(legend.position = "none", panel.grid.major.y = element_line(), 
...
```{r}
ggplot(exp4.gmm.specificity.arm.3comp, aes(x=n.probes, y=tnr, color = cn.sim.class)) +
    geom_point(alpha = 0.5, position = position_jitter(width = 0.25)) + 
    theme_classic_custom +
    theme(legend.position = "top", panel.grid.major = element_line()) +
    theme(legend.box.margin = margin(0, 10, 0, 10), legend.margin = margin(0), plot.margin = unit(c(0, 5.5, 5.5, 5.5), "pt")) +
...
```{r}
exp4.gmm.metrics.table <- exp4.gmm.metrics %>% mutate(feature.id = paste0("chr", feature.id)) %>% rbind(exp4.gmm.metrics.arms) %>% select(feature.id, cn.sim.class, true.pos.p) %>% mutate(true.pos.p = round(true.pos.p, 2))
exp4.gmm.metrics.table <- exp4.gmm.metrics.table %>% pivot_wider(names_from = cn.sim.class, values_from = true.pos.p)
```

```{r}
exp4.gmm.metrics.3.table <- exp4.gmm.metrics.3 %>% mutate(feature.id = paste0("chr", feature.id))
exp4.gmm.metrics.arm.3comp
```

```{r}
sensitivity.gain <- exp4.gmm.metrics.3.table %>% filter(cn.sim.class == "sim_cn3") %>% pull(true.pos.p) %>% round(.,2) %>% set_names(NULL)
sensitivity.gain <- c(sensitivity.gain, exp4.gmm.metrics.arm.3comp %>% filter(cn.sim.class == "sim_cn3") %>% pull(true.pos.p) %>% round(.,2) %>% set_names(NULL))
exp4.gmm.metrics.table$gain <- sensitivity.gain
```

```{r}
write.csv(x = exp4.gmm.metrics.table, file = here("figures", "figure-i", "exp04.theoretical.sensitivity.csv"), quote = F, row.names = F)
```
