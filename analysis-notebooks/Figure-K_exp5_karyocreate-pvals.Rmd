...
calc.fisher.raw <- function(grna){
    ...
    cn.fisher$p.adj.rounded <- round(cn.fisher$p.val, 3)
    return(cn.fisher)
}

calc.fisher.bh <- function(grna){
    cn.all <- cn.gmm.results.collapse %>% select(feature.id, gmmCopyNumber, sample.call) %>% group_split(.by = feature.id, .keep = F)
    cn.fisher <- data.frame(chr = c(1:22, "X"))
    cn.fisher$p.val <- cn.all %>% map(function(x){
        x <- x %>% dplyr::select(gmmCopyNumber, sample.call) %>% filter(sample.call %in% c("gNC", grna))
...
```{r}
fisher.result <- map(grna.levels, ~calc.fisher.bon(.x))
...
fisher.result.raw <- fisher.result.raw %>% dplyr::select(sample, chr, p.adj.rounded) %>% pivot_wider(names_from = sample, values_from = p.adj.rounded) %>% print()
```

BH/FDR Corrected
```{r}
fisher.result.bh <- map(grna.levels, ~calc.fisher.bh(.x))
fisher.result.bh <- fisher.result.bh %>% list_rbind(names_to = "sample")
fisher.result.bh$sample <- factor(fisher.result.bh$sample, levels = unique(fisher.result.bh$sample), labels = grna.levels)
fisher.result.bh <- fisher.result.bh %>% dplyr::select(sample, chr, p.adj.rounded) %>% pivot_wider(names_from = sample, values_from = p.adj.rounded) %>% print()
```

```{r}
cn.gmm.results.collapse2 <- cn.gmm.results.collapse
cn.gmm.results.collapse2$gmmCopyNumber <- fct_collapse(cn.gmm.results.collapse2$gmmCopyNumber, "aneuploid" = c("3+",  "1"))
cn.gmm.results.collapse2$gmmCopyNumber <- fct_recode(cn.gmm.results.collapse2$gmmCopyNumber, "diploid" = "2")
```

```{r}
calc.fisher.bon2 <- function(grna){
    cn.all <- cn.gmm.results.collapse2 %>% select(feature.id, gmmCopyNumber, sample.call) %>% group_split(.by = feature.id, .keep = F)
    cn.fisher <- data.frame(chr = c(1:22, "X"))
    cn.fisher$p.val <- cn.all %>% map(function(x){
        x <- x %>% dplyr::select(gmmCopyNumber, sample.call) %>% filter(sample.call %in% c("gNC", grna))
        x$sample.call <- fct_drop(x$sample.call)
...
calc.fisher.raw2 <- function(grna){
    cn.all <- cn.gmm.results.collapse2 %>% select(feature.id, gmmCopyNumber, sample.call) %>% group_split(.by = feature.id, .keep = F)
    cn.fisher <- data.frame(chr = c(1:22, "X"))
    cn.fisher$p.val <- cn.all %>% map(function(x){
        x <- x %>% dplyr::select(gmmCopyNumber, sample.call) %>% filter(sample.call %in% c("gNC", grna))
...
Bonferroni Corrected
```{r}
fisher.result2 <- map(grna.levels, ~calc.fisher.bon2(.x))
fisher.result2 <- fisher.result2 %>% list_rbind(names_to = "sample")
fisher.result2$sample <- factor(fisher.result2$sample, levels = unique(fisher.result2$sample), labels = grna.levels)
fisher.result2 <- fisher.result2 %>% dplyr::select(sample, chr, p.adj.rounded) %>% pivot_wider(names_from = sample, values_from = p.adj.rounded) %>% print()
```

Raw p-val
```{r}
fisher.result.raw2 <- map(grna.levels, ~calc.fisher.raw2(.x))
fisher.result.raw2 <- fisher.result.raw2 %>% list_rbind(names_to = "sample")
fisher.result.raw2$sample <- factor(fisher.result.raw2$sample, levels = unique(fisher.result.raw2$sample), labels = grna.levels)
fisher.result.raw2 <- fisher.result.raw2 %>% dplyr::select(sample, chr, p.adj.rounded) %>% pivot_wider(names_from = sample, values_from = p.adj.rounded) %>% print()
```

BH/FDR Corrected
```{r}
fisher.result.bh2 <- map(grna.levels, ~calc.fisher.bh2(.x))
fisher.result.bh2 <- fisher.result.bh2 %>% list_rbind(names_to = "sample")
fisher.result.bh2$sample <- factor(fisher.result.bh2$sample, levels = unique(fisher.result.bh2$sample), labels = grna.levels)
fisher.result.bh2 <- fisher.result.bh2 %>% dplyr::select(sample, chr, p.adj.rounded) %>% pivot_wider(names_from = sample, values_from = p.adj.rounded) %>% print()
```
...
cn.gmm.results.collapse.prop %>% 
    select(!n) %>% 
    pivot_wider(id_cols = c("sample.call", "gmmCopyNumber"), names_from = feature.id, values_from = cn.prop) %>% 
write.table(., file = here("figures", "figure-k", "exp05.kc.proportions.txt"), col.names = T, row.names = F, quote = F, sep = "\t")
```
...
```{r}
fisher.result.figure <- pivot_longer(fisher.result.bh2, starts_with("g"), values_to = "pval", names_to = "sample.call")
...
fisher.result.figure$pval[fisher.result.figure$pval > 0.1] <- ""
```
...
cn.gmm.results.collapse.prop.compare %>% filter(sample.call != "gNC") %>% 
    ...
    geom_text(data = fisher.result.figure %>% filter(feature.id != 18), aes(label = pval, x = feature.id, y = -20), inherit.aes = F, size = 6/.pt) + 
...
colnames(fisher.result.table)[2:3] <- c("sample", "p.value")
```

```{r}
write.table(fisher.result.table, file = here("figures", "figure-k", "exp05.fisher.results.txt"), quote = F, sep = "\t", row.names = F, col.names = T)
```
