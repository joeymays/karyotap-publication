...
gmm1 <- plotCopyNumberGMM(exp1, feature.id = 1, draw.boundaries = T) +     
    ...
    scale_y_continuous(expand = c(0,0,0,0.05), breaks = seq(0, 1, 0.25), labels = c("0.0", "", "0.5", "", "1.0")) +
...
gmm2 <- plotCopyNumberGMM(exp1, feature.id = 13, draw.boundaries = T) +
        theme_classic_custom +
        theme(legend.position = "none", panel.grid.minor = element_blank(), panel.grid.major.y = element_blank(),
          plot.margin = unit(c(0, 5.5, 5.5, 0), "pt"), axis.title = element_blank()) + 
    labs(title = NULL, color = "Copy Number Component") +
    scale_y_continuous(expand = c(0,0,0,0.05), breaks = seq(0, 1, 0.25), labels = c("0.0", "", "0.5", "", "1.0")) +
    scale_x_continuous(expand = c(0,0,0,0), breaks = 1:10) +
...
gmm4 <- plotCopyNumberGMM(exp1, feature.id = "X", draw.boundaries = T) +
        theme_classic_custom +
    theme(legend.position = "none", panel.grid.minor = element_blank(), panel.grid.major.y = element_blank(),
          plot.margin = unit(c(0, 5.5, 5.5, 0), "pt"), axis.title.y = element_blank()) + 
    labs(title = NULL, color = "Copy Number Component", x = "Copy Number Score") +
    scale_y_continuous(expand = c(0,0,0,0.05), breaks = seq(0, 1, 0.25), labels = c("0.0", "", "0.5", "", "1.0")) +
    scale_x_continuous(expand = c(0,0,0,0), breaks = 1:10) +
...
gmm1.legend <- gmm1 + theme(legend.box.margin = margin(0, 0, 5, 0), legend.position = "top") + guides(color = guide_legend(nrow = 1))

gmm.legend <- get_plot_component(gmm1.legend, "guide-box-top", return_all = T)
...
n.probes <- fct_count(rowData(exp1)$chr) #n.probes reflects probes after filtering
...
ggplot(rpe1.data.results, aes(x=n.probes, y=pct.concordance)) +
    ...
    labs(y= "Accuracy (%)", x = "Number of Probes per Chromosome") +
    theme_classic_custom + 
    theme(panel.grid.major = element_line()) +
...
ggplot(exp1.gmm.metrics, aes(x=n.probes, y=true.pos.p, color = cn.sim.class)) +
    geom_point(alpha = 0.5, position = position_jitter(width = 0.25), pch = 16) + 
    theme_classic_custom +
...
ggplot(rpe1.data.arm.results, aes(x=n.probes, y=pct.concordance)) +
    ...
    labs(y= "Accuracy (%)", x = "Number of Probes per Chromosome Arm") +
    theme_classic_custom + 
    theme(panel.grid.major = element_line(),
...
```{r}
exp1.gmm.metrics.table <- exp1.gmm.metrics %>% mutate(feature.id = paste0("chr", feature.id)) %>% rbind(exp1.gmm.metrics.arm) %>% select(feature.id, cn.sim.class, true.pos.p) %>% mutate(true.pos.p = round(true.pos.p, 2))
exp1.gmm.metrics.table <- exp1.gmm.metrics.table %>% pivot_wider(names_from = cn.sim.class, values_from = true.pos.p)

write.table(x = exp1.gmm.metrics.table, file = here("figures", "figure-c", "exp01.theoretical.sensitivity.txt"), quote = F, sep = '\t', row.names = F, col.names = T)
```
...
ggplot(exp1.gmm.metrics.arm, aes(x=n.probes, y=true.pos.p, color = cn.sim.class)) +
    ...
    scale_y_continuous(limits = c(12, 100), expand = c(0,0,0,0))
tpr.arm.nprobes
```
...
ggplot(chr.lengths, aes(x=length.bp/10^8, y=pct.concordance)) +
    ...
    labs(y= "Accuracy (%)", x = "Chromosome Length (100 MB)") +
    theme_classic_custom + 
    theme(panel.grid.major = element_line()) +
    scale_y_continuous(limits = c(30,100), expand = c(0,0,0,0)) +
    annotate("text", x = 2.48, y = 37, size = 7/.pt, hjust = 1, label = paste0("Chr Length vs Accuracy , Pearson r = ", round(length.corr, 2))) +
