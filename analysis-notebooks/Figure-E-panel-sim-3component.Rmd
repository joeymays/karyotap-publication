---
title: "Figure E Simulation - 3 component model"
author: "Joey Mays"
date: '2023-01-17'
output:
  pdf_document:
...
```{r}
here::i_am("analysis-notebooks/Figure-E-panel-sim-3component.Rmd")
```
```{r}
...
add.probe.simulation <- function(idx, probe.sample) {
  ...
  for (i in 4:ncol(probe.sample)) {
    ...
    x <- CNweaveR:::.createTapestriExperiment.sim(counts = sim.ploidy.t, probe.metadata = probe.metadata)
    ...
    x <- suppressMessages(calcGMMCopyNumber(x, cell.barcodes = colnames(x), control.copy.number = control.cn, model.components = 1:3))
...
```{r}
saveRDS(add.simulation.results, file = here("datasets", "scramble.probe.sim.results.3component.RDS"))
```
