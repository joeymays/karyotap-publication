...
downsample.sequence <- c(24, downsample.sequence)
```
```{r}
...
for(i in seq_len(length(rpe1.probe.samples))){
  ...
  te <- CNweaveR:::.createTapestriExperiment.sim(counts = current.mat, probe.metadata = row.data)
...
sim.step <- function(tapestri.object.init, step, n.reps){
    ...
    tapestri.object <- suppressMessages(calcSmoothCopyNumber(tapestri.object.init)) #smooth copy number
  control.cn <- generateControlCopyNumberTemplate(tapestri.object, copy.number = 2)
