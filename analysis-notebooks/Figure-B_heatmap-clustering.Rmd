...
```{r eval=FALSE}
png(filename = here("figures", "figure-b", "exp01.af.umap.png"), 
...
```{r}
scatter.raw$cluster <- fct_collapse(scatter.raw$cluster, doublet = setdiff(levels(scatter.raw$cluster), 1:5))
scatter.raw$cluster <- fct_recode(scatter.raw$cluster, LS513 = "1", RPE1 = "2", SW48 = "3", LoVo = "4", CL11 = "5")
...
```{r}
probe.copynumber.hm <- assayHeatmap(exp1, assay = "copyNumber", split.col.by = "arm", split.row.by = "cluster", color.custom = circlize::colorRamp2(c(1, 1.5, 2, 2.5, 3, 4), c("#2c7bb6", "#ffffff", "#ffffff", "#ffffff", "#fdae61", "#d7191c")), 
            heatmap_legend_param = list(title_position = "lefttop-rot", border = "black"), name = "Copy Number")
probe.copynumber.hm
```
...
pdf(file = here("figures", "figure-b", "exp01.heatmap.pdf"), 
    width = 7.7, height = 3.2)
assayHeatmap(exp1, assay = "copyNumber", split.col.by = "arm", split.row.by = "cluster", color.custom = circlize::colorRamp2(c(1, 1.5, 2, 2.5, 3, 4), c("#2c7bb6", "#ffffff", "#ffffff", "#ffffff", "#fdae61", "#d7191c")), 
            heatmap_legend_param = list(title_position = "lefttop-rot", border = "black"), name = "Copy Number", use_raster = TRUE, raster_quality = 7)
dev.off()
```
